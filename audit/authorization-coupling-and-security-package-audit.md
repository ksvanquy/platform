...
| **SEC-501** | Tổng duyệt toàn diện E2E Suite & Build Verification | `/tests/e2e-quiz-decomposition.spec.ts`, `/vitest.config.ts` | - Chạy toàn bộ chuỗi kịch bản E2E từ Login, Quản lý câu hỏi, Tạo đề, Làm bài, Nộp bài và Xem kết quả. | `npm test` pass toàn bộ 36 spec files (296 tests). Không còn bất kỳ file nào ngoài `services/auth` import `getDefaultRsaKeyPair`. |
| **SEC-502** | Benchmark Hiệu năng Dual-Mode & Security Hardening | `/tests/security-dual-mode-hardening.spec.ts`, `/packages/security/src/middlewares/*` | - Benchmark CPU Strict RSA vs Mesh-Trust.<br/>- Kiểm thử chống giả mạo header (Anti-spoofing), tamper-proofing, expired tokens, pure ABAC ownership. | Tiết kiệm >60% CPU thời gian xử lý tại downstream services (Mesh mode). Chống giả mạo 100% tại ranh giới Strict / Edge Gateway. |
