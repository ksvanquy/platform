...
export function isExamDbConfigured(): boolean {
  return Boolean(getExamDatabaseUrl());
}
...
export function getExamDb() {
  ...
  const connectionString = getExamDatabaseUrl();
  if (!connectionString) {
    throw new Error(
      'FATAL ERROR: EXAM_DATABASE_URL is not defined in environment variables. ' +
      'In-memory persistence has been permanently removed; PostgreSQL (exam_db) is strictly required.'
    );
  }
...
export async function closeExamDb(): Promise<void> {
  if (sqlClient) {
    await sqlClient.end();
    sqlClient = null;
    dbInstance = null;
