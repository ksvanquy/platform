...
export function getTaxonomyDatabaseUrl(): string | undefined {
  loadEnvIfAvailable();
  const url = (process.env.TAXONOMY_DATABASE_URL || process.env.DATABASE_URL)?.trim();
  if (!url || url === 'TAXONOMY_DATABASE_URL' || url === 'DATABASE_URL') return undefined;
...
export function isTaxonomyDbConfigured(): boolean {
  return Boolean(getTaxonomyDatabaseUrl());
}

/**
 * Lấy đối tượng kết nối Drizzle ORM tới taxonomy_db (Singleton)
 * Thực thi nguyên tắc Fail-Fast nếu chưa có biến môi trường
 */
export function getTaxonomyDb() {
  if (dbInstance) {
    return dbInstance;
  }

  const connectionString = getTaxonomyDatabaseUrl();
  if (!connectionString) {
    throw new Error(
      'FATAL ERROR: TAXONOMY_DATABASE_URL is not defined in environment variables. ' +
      'PostgreSQL (taxonomy_db) is strictly required.'
    );
  }
...
export async function closeTaxonomyDb(): Promise<void> {
  if (sqlClient) {
    await sqlClient.end();
    sqlClient = null;
    dbInstance = null;
